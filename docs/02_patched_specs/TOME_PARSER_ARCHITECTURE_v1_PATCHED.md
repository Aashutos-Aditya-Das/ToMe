...
## CORE PARSER INTERFACES (Canonical)
...
export interface ASTDependencyEdge {
  sourceId: string; // FQN
  targetId: string; // FQN or external package
  type: 'IMPORTS' | 'IMPLEMENTS' | 'EXTENDS' | 'CALLS';
}

export interface FileNode {
  path: string; // Relative to workspace root
  hash: string; // StructuralChecksum
  symbols: ExtractedSymbol[];
  dependencies: ASTDependencyEdge[];
}

/**
 * RUNTIME-ONLY: This model is never serialized or persisted.
 */
