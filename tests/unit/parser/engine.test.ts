...
describe('ParserEngine', () => {
  ...
  test('parses a workspace up to 500 files and throws on > 500', async () => {
    const paths = Array.from({ length: 501 }, (_, i) => `file${i}.ts`);
    await expect(engine.parseWorkspace(paths)).rejects.toThrow(/exceeds maximum supported size/);
  });
